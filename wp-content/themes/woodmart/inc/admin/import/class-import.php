...
class Import extends Singleton {
	...
	public function render() {
		...
		if ( $this->is_imported( 'base' ) || $this->is_imported( 'megamarket_base' ) ) {
		...
		?>" data-current-base="<?php echo esc_attr( $current_base ); ?>">
			...
			<div class="woodmart-box-content">
				...
				<div class="wd-import-items<?php echo esc_attr( $items_classes ); ?>">
					<?php foreach ( $this->version_list as $slug => $version_data ) : ?>
						...
						?>
						<div class="wd-import-item<?php echo esc_attr( $item_classes ); ?>"
							data-version="<?php echo esc_attr( $slug ); ?>" data-base="<?php echo esc_attr(
							$base ); ?>" data-type="<?php echo esc_attr( $type ); ?>" data-tags="<?php echo esc_attr( $tags ); ?>">
	...
	private function print_notices() {
		if ( $this->get_required_plugins() ) {
			$this->print_notice( 'warning', sprintf( __( 'برای درون ریزی ابتدا باید این افزونه ها را نصب کنید: <strong><a href="%1$s">%2$s</a></strong>', 'woodmart' ), esc_url( add_query_arg( 'page', rawurlencode( 'tgmpa-install-plugins' ), self_admin_url( 'themes.php' ) ) ), implode( ', ', $this->get_required_plugins() ) ) );
		}

		if ( defined( 'ELEMENTOR_VERSION' ) && defined( 'WPB_PLUGIN_DIR' ) ) {
			$this->print_notice( 'warning', __( 'لطفا فقط یکی از افزونه های <strong>WPBakery page builder</strong> یا <strong>Elementor</strong> را فعال نگه دارید تا در سایت شما تداخلی ایجاد نشود..', 'woodmart' ) );
		}
	}
	...
	private function set_versions_list() {
		...
		unset( $this->version_list['megamarket_base'] );
